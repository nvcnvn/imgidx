...
func average(sum []uint64, w, h int, n uint64) tipimage.Image {
	ret := tipimage.NewRGBA(tipimage.Rect(0, 0, w, h))
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			index := 4 * (y*w + x)
...
// resizeYCbCr returns a scaled copy of the YCbCr tipimage slice r of m.
// The returned tipimage has width w and height h.
func resizeYCbCr(m *tipimage.YCbCr, r tipimage.Rectangle, w, h int) (tipimage.Image, bool) {
	var verticalRes int
	switch m.SubsampleRatio {
	case tipimage.YCbCrSubsampleRatio420:
		verticalRes = 2
	case tipimage.YCbCrSubsampleRatio422:
		verticalRes = 1
	default:
		return nil, false
	}
...
// resizeRGBA returns a scaled copy of the RGBA tipimage slice r of m.
// The returned tipimage has width w and height h.
func resizeRGBA(m *tipimage.RGBA, r tipimage.Rectangle, w, h int) tipimage.Image {
	ww, hh := uint64(w), uint64(h)
	dx, dy := uint64(r.Dx()), uint64(r.Dy())
	// See comment in Resize.
	n, sum := dx*dy, make([]uint64, 4*w*h)
